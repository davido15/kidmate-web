...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($class_name) && !empty($class_code)) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            $query = "INSERT INTO classes (class_name, class_code, description, is_active) VALUES (?, ?, ?, ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("sssi", $class_name, $class_code, $description, $is_active);
            
            if ($stmt->execute()) {
                header("Location: manage_classes.php?success=Class added successfully!");
                exit;
            } else {
                $errorMessage = "Failed to add class: " . $stmt->error;
            }
            $stmt->close();
        }
        $check_stmt->close();
                                    ...
                                    <div class="col-xl-6">
                                        <div class="mb-3">
                                            ...
                                            <textarea name="description" class="form-control" rows="3" 
                                                      placeholder="Enter class description"><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
                                        </div>
