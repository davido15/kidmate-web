...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($subject_name) && !empty($subject_code)) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            $query = "UPDATE subjects SET subject_name = ?, subject_code = ?, description = ?, is_active = ? WHERE id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("sssii", $subject_name, $subject_code, $description, $is_active, $id);
            
            if ($stmt->execute()) {
                header("Location: manage_subjects.php?success=Subject updated successfully!");
                exit;
            } else {
                $errorMessage = "Failed to update subject: " . $stmt->error;
            }
            $stmt->close();
        }
        $check_stmt->close();
                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Edit Subject: <?php echo htmlspecialchars($subject['subject_name']); ?></h4>
                        </div>
                        <div class="card-body">
                                    ...
                                    <div class="col-xl-6">
                                        <div class="mb-3">
                                            ...
                                            <textarea name="description" class="form-control" rows="3" 
                                                      placeholder="Enter subject description"><?php echo htmlspecialchars($_POST['description'] ?? $subject['description'] ?? ''); ?></textarea>
                                        </div>

                                        <div class="mb-3">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" 
                                                       <?php echo (($_POST['is_active'] ?? $subject['is_active']) ? 'checked' : ''); ?>>
