...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($term_name) && !empty($term_code) && !empty($start_date) && !empty($end_date)) {
        ...
        if (strtotime($start_date) >= strtotime($end_date)) {
            ...
            // Check if term code already exists for other terms
            $check_query = "SELECT id FROM terms WHERE term_code = ? AND id != ?";
            $check_stmt = $conn->prepare($check_query);
            $check_stmt->bind_param("si", $term_code, $id);
            $check_stmt->execute();
            ...
            if ($check_result->num_rows > 0) {
                ...
                $query = "UPDATE terms SET term_name = ?, term_code = ?, start_date = ?, end_date = ?, is_active = ? WHERE id = ?";
                $stmt = $conn->prepare($query);
                $stmt->bind_param("ssssii", $term_name, $term_code, $start_date, $end_date, $is_active, $id);
                
                if ($stmt->execute()) {
                    header("Location: manage_terms.php?success=Term updated successfully!");
                    exit;
                } else {
                    $errorMessage = "Failed to update term: " . $stmt->error;
                }
                $stmt->close();
            }
            $check_stmt->close();
                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Edit Term: <?php echo htmlspecialchars($term['term_name']); ?></h4>
                        </div>
                        <div class="card-body">
                                    ...
                                    <div class="col-xl-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="term_name" class="form-control" required 
                                                   value="<?php echo htmlspecialchars($_POST['term_name'] ?? $term['term_name']); ?>"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="term_code" class="form-control" required 
                                                   value="<?php echo htmlspecialchars($_POST['term_code'] ?? $term['term_code']); ?>"
                                    ...
                                    <div class="col-xl-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="date" name="start_date" class="form-control" required 
                                                   value="<?php echo htmlspecialchars($_POST['start_date'] ?? $term['start_date']); ?>">
                                        </div>

                                        <div class="mb-3">
                                            <label class="form-label">End Date <span class="text-danger">*</span></label>
                                            <input type="date" name="end_date" class="form-control" required 
                                                   value="<?php echo htmlspecialchars($_POST['end_date'] ?? $term['end_date']); ?>">
                                        </div>

                                        <div class="mb-3">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" 
                                                       <?php echo (($_POST['is_active'] ?? $term['is_active']) ? 'checked' : ''); ?>>
